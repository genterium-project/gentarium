...
Gentarium
    if(strType == "GentariumNetworkVariable") type = GentariumNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == GentariumNetwork) s = "GentariumNetwork";
    if(type == GentariumNetworkVariable) s = "GentariumNetworkVariable";
