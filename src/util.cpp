...
const char * const BITCOIN_PID_FILENAME = "gentariumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gentarium" is a composite category enabling all Gentarium-related debug output
            if(ptrCategory->count(string("gentarium"))) {
...
#else
    const char* pszModule = "gentarium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GentariumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GentariumCore
    // Mac: ~/Library/Application Support/GentariumCore
    // Unix: ~/.gentariumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GentariumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GentariumCore";
#else
    // Unix
    return pathRet / ".gentariumcore";
#endif
#endif
}
